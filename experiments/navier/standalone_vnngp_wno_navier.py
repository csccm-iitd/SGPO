#!/usr/bin/env python
"""Standalone Navier-Stokes experiment — VNNGP + WNO mean + WNO kernel embedding.

    curl(f) -> omega   (2-D vorticity)

Usage:
    python experiments/navier/standalone_vnngp_wno_navier.py
"""
...
def main():
    ...
    print("=" * 70)
    print("  Navier-Stokes — VNNGP + WNO mean + WNO kernel embedding")
    ...
    model = VNNGP(
        ...
        num_nn=NUM_NN, num_inducing=NUM_INDUCING,
    ...
    np.savez(os.path.join(SAVE_DIR, f"vnngp_wno_{ts}.npz"),
    ...
    torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"vnngp_wno_{ts}.pt"))
    print(f"  Saved to {SAVE_DIR}/vnngp_wno_{ts}.*")
