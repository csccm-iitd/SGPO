#!/usr/bin/env python
"""Standalone Navier-Stokes experiment — Exact GP + WNO mean (no kernel embedding).

    curl(f) -> omega   (2-D vorticity)

Usage:
    python experiments/navier/standalone_exactgp_wno_navier.py
"""
...
def main():
    ...
    print("=" * 70)
    print("  Navier-Stokes — Exact GP + WNO mean")
    print(f"  Device: {device} | Epochs: {NUM_EPOCHS}")
    print(f"  WNO width={WNO_WIDTH}, layers={WNO_LAYERS}")
    print("=" * 70)

    r = SUB
    raw_in = torch.tensor(np.load(INPUT_PATH)).permute(2, 1, 0).float()
    raw_out = torch.tensor(np.load(OUTPUT_PATH)).permute(2, 1, 0).float()

    x_train = raw_in[:NTRAIN, ::r, ::r][:, :s, :s]
    y_train = raw_out[:NTRAIN, ::r, ::r][:, :s, :s]
    x_test = raw_in[NTRAIN:NTRAIN+NTEST, ::r, ::r][:, :s, :s]
    y_test = raw_out[NTRAIN:NTRAIN+NTEST, ::r, ::r][:, :s, :s]
    ...
    wno = WNO2d(width=WNO_WIDTH, level=WNO_LEVEL, layers=WNO_LAYERS,
    ...
    if y_normalizer is not None:
        ...
        pred_mean = pred_mean.reshape(-1, *RESOLUTION)
        pred_mean = y_normalizer.decode(pred_mean).reshape(-1, num_tasks)
        pred_var = pred_var.reshape(-1, *RESOLUTION)
        pred_var = (pred_var * std**2).reshape(-1, num_tasks)

    pred_mean = pred_mean.cpu(); pred_var = pred_var.cpu()
